# Synchronization : Lock(Mutex), Semaphore
...
- Counting Sempaphore
    ...
    세마포어의 카운트는 보통 가용한 자원의 수(또는 수-1)로 초기화한다.
...
**세마포어를 활용하면 쓰레드간 순서를 지정하여 구현할 수 있다.** 쓰레드 A->B 순으로 실행되어야 하는 작업의 경우, 세마포어 값을 0으로 두고 A,B 순으로 실행하면 된다. -> binary sempahore